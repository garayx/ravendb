        ...
        public static async Task Main(string[] args)
        {
            ...
            for (int i = 0; i < 10_000; i++)
            {
                ...
                try
                {
                    ...
                    using (var test = new RavenDB_14687(testOutputHelper))
                    {
                        //await test.RemoveNodeFromDatabaseGroupWhileRollingDeployment();
                      await  test.IndexSpecificSettingShouldBeRespected(RavenTestBase.Options.ForMode(RavenDatabaseMode.Single, RavenJavascriptEngineMode.Jint));
                    }
                }
                catch (Exception e)
                {
