...
public partial class RavenTestBase
{
    public class ShardedSubscriptionTestBase
    {
        ...
        public async Task AssertNoItemsInTheResendQueueAsync(IDocumentStore store, string subscriptionId, List<RavenServer> servers = null)
        {
            var id = long.Parse(subscriptionId);
            var shards = _parent.Sharding.GetShardsDocumentDatabaseInstancesFor(store, servers);
