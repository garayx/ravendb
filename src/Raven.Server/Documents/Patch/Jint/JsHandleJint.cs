...
public struct JsHandleJint : IJsHandle<JsHandleJint>
{
    ...
    public JSValueType ValueType
    {
        ...
        get
        {
            ...
            if (Item.IsObject())
            {
                //TODO: egor check if shlomo jint update made those public?
                ...
                if (obj is DateInstance) // TODO: egor now its  JsDate?
                    return JSValueType.Date;
    ...
    public void FastAddProperty(string name, JsHandleJint value, bool writable, bool enumerable, bool configurable)
    {
        if (_obj == null)
            throw new NotSupportedException($"Not supported for non object value.");
        _obj.FastSetProperty(name, new PropertyDescriptor(value.Item, writable, enumerable, configurable));
    }
    ...
    public void FastSetDataProperty(string name, JsHandleJint value)
    {
        if (_obj == null)
            throw new NotSupportedException($"Not supported for non object value.");

        _obj.FastSetDataProperty(name, value.Item);
    }
