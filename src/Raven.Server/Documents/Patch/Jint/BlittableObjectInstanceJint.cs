...
namespace Raven.Server.Documents.Patch.Jint
{
    ...
    public sealed class BlittableObjectInstanceJint : ObjectInstance, IBlittableObjectInstance<JsHandleJint>
    {
        ...
        public BlittableJsonReaderObject Blittable { get; set; }
        public string DocumentId { get; set; }
        public Dictionary<string, BlittableJsonToken> OriginalPropertiesTypes { get; set; }
        public Lucene.Net.Documents.Document LuceneDocument { get; set; }
        public IState LuceneState { get; set; }
        public Dictionary<string, IndexField> LuceneIndexFields { get; set; }
        public bool LuceneAnyDynamicIndexFields { get; set; }

        public HashSet<string> Deletes;
        public Dictionary<string, BlittableObjectProperty> _ownValues;
        ...
        {
            value = null;
            isDeleted = Deletes?.Contains(propertyName) == true;
            if (isDeleted)
                return false;
        ...
        public ObjectInstance GetOrCreateJint(string key)
        {
            ...
            _ownValues ??= new Dictionary<string, BlittableObjectProperty>(Blittable.Count);
        ...
        public sealed class BlittableObjectProperty : PropertyDescriptor, IBlittableObjectProperty<JsHandleJint>
        {
                ...
                : base(PropertyFlag.CustomJsValue | PropertyFlag.Writable | PropertyFlag.WritableSet | PropertyFlag.Enumerable | PropertyFlag.EnumerableSet)
            {
                ...
                if (TryGetValueFromLucene(_parent, _property, out _value) == false)
                {
                    ...
                    if (TryGetValueFromDocument(_parent, _property, out _value) == false)
                    {
                        ...
                        {
                            if (_parent._projection.MustExtractOrThrow)
                                _parent._projection.ThrowCouldNotExtractFieldFromDocumentBecauseDocumentDoesNotContainSuchField(_parent.DocumentId, property);
                        }
            ...
            private JsArray GetArrayInstanceFromBlittableArray(Engine e, BlittableJsonReaderArray bjra, BlittableObjectInstanceJint parent)
            {
                bjra.NoCache = true;
        ...
        public override bool Delete(JsValue property)
        {
            ...
            MarkChanged();
            Deletes.Add(name);
            return _ownValues?.Remove(name) == true;
        }
        ...
        {
            var list = new List<JsValue>(Blittable?.Count ?? _ownValues?.Count ?? 0);
            ...
            foreach (var prop in Blittable.GetPropertyNames())
            {
                if (Deletes?.Contains(prop) == true)
                    continue;
                if (_ownValues != null && _ownValues.ContainsKey(prop))
                    continue;
