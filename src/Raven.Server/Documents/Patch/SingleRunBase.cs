    ...
    where T : struct, IJsHandle<T>
{
    ...
    public T TimeSeries(T self, T[] args)
    {
        ...
        var obj = EngineHandle.CreateObject();
        obj.FastSetDataProperty("append", EngineHandle.CreateClrCallBack("append", AppendTimeSeries));
        obj.FastSetDataProperty("increment", EngineHandle.CreateClrCallBack("increment", IncrementTimeSeries));
        obj.FastSetDataProperty("delete", EngineHandle.CreateClrCallBack("delete", DeleteRangeTimeSeries));
        obj.FastSetDataProperty("get", EngineHandle.CreateClrCallBack("get", GetRangeTimeSeries));
        obj.FastSetDataProperty("getStats", EngineHandle.CreateClrCallBack("getStats", GetStatsTimeSeries));
    ...
    private T GetRangeTimeSeries(T self, T[] args)
    {
        ...
        foreach (var singleResult in reader.AllValues())
        {
            ...
            entry.FastSetDataProperty(nameof(TimeSeriesEntry.Timestamp), EngineHandle.CreateValue(singleResult.Timestamp.GetDefaultRavenFormat(isUtc: true)));
            entry.FastSetDataProperty(nameof(TimeSeriesEntry.Tag), singleResult.Tag == null ? EngineHandle.Null : EngineHandle.CreateValue(singleResult.Tag.ToString()));
            entry.FastSetDataProperty(nameof(TimeSeriesEntry.Values), EngineHandle.CreateArray(jsSpanItems));
            entry.FastSetDataProperty(nameof(TimeSeriesEntry.IsRollup), EngineHandle.CreateValue(singleResult.Type == SingleResultType.RolledUp));
    ...
    private T GetStatsTimeSeries(T self, T[] args)
    {
        ...
        tsStats.FastSetDataProperty(nameof(stats.Start), EngineHandle.CreateValue(stats.Start));
        tsStats.FastSetDataProperty(nameof(stats.End), EngineHandle.CreateValue(stats.End));
        tsStats.FastSetDataProperty(nameof(stats.Count), EngineHandle.CreateValue(stats.Count));
        return tsStats;
    }
