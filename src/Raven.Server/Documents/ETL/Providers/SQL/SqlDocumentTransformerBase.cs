...
{
    public abstract class SqlDocumentTransformerBase<T> : EtlTransformer<ToSqlItem, SqlTableWithRecords, EtlStatsScope, EtlPerformanceOperation, T>
        where T : struct, IJsHandle<T>
    {
        ...
        protected SqlDocumentTransformerBase(Transformation transformation, DocumentDatabase database, DocumentsOperationContext context, SqlEtlConfiguration config)
        ...
        private T ToVarcharTranslator(T type, T[] args)
        {
            ...
            item.FastSetDataProperty(nameof(VarcharFunctionCall.Size), sizeSpecified ? args[1] : EngineHandle.CreateValue(DefaultVarCharSize));
        ...
        protected override void LoadToFunction(string tableName, ScriptRunnerResult<T> cols)
        {
            if (tableName == null)
