    ...
    where T : struct, IJsHandle<T>
{
    ...
    public abstract RavenEtlScriptRun<T> CreateRavenEtlScriptRun(EtlStatsScope stats, Transformation transformation);

    public override void Transform(RavenEtlItem item, EtlStatsScope stats, EtlProcessState state)
    {
        Current = item;
        _currentRun ??= CreateRavenEtlScriptRun(stats, _transformation);
