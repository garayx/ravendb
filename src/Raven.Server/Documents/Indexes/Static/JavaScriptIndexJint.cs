...
{
    public JavaScriptIndexJint(IndexDefinition definition, RavenConfiguration configuration, long indexVersion) : base(definition, configuration,
        modifyMappingFunctions: null, JavaScriptIndexHelper.GetMapCode(), indexVersion)
    {
    }
        ...
        out Dictionary<string, Dictionary<string, List<JavaScriptMapOperation<JsHandleJint>>>> collectionFunctions)
    {
        ...
        var maps = _definitions.GetProperty(MapsProperty);
        ...
        for (int i = 0; i < mapCount; i++)
        {
            ...
            var map = maps.GetProperty(i);
            if (map.IsObject == false)
