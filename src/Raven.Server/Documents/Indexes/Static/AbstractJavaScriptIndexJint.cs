...
public abstract class AbstractJavaScriptIndexJint : AbstractJavaScriptIndex<JsHandleJint>
{
        ...
        : base(definition)
    {
        ...
        //TODO: egor we need to use SingleIndexConfiguration (it was removed in orig v8 repo somewhy)
        var indexConfiguration = new SingleIndexConfiguration(definition.Configuration, configuration);
        var MaxStepsForScript = configuration.Indexing.MaxStepsForScript;
        var x1 = configuration.JavaScript.StrictMode;
        var x12 = configuration.JavaScript.MaxDuration;
        var x123 = configuration.JavaScript.TargetContextCountPerEngine; // redundant ...
        var x1234 = configuration.JavaScript.MaxSteps; // should be MaxStepsForScript (or the other variant we had)
