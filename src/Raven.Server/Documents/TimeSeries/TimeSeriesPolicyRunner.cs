...
namespace Raven.Server.Documents.TimeSeries
{
    public sealed class TimeSeriesPolicyRunner : BackgroundWorkBase
    {
            ...
            catch (Exception e)
            {
                ...
                catch (Exception ex)
                {
                    if (logger.IsWarnEnabled)
                        logger.Warn("Failed to dispose previous time-series policy runner", ex);
                }
        ...
        internal async Task<long> RunRollups(bool propagateException = true, List<string> explanations = null)
        {
            ...
            {
                if (Logger.IsErrorEnabled)
                    Logger.Error($"Failed to roll-up time series for '{_database.Name}' which are older than {now}", e);
        ...
        internal async Task DoRetention(bool propagateException = true)
        {
            ...
            {
                if (Logger.IsErrorEnabled)
                    Logger.Error($"Failed to execute time series retention for database '{_database.Name}'", e);
