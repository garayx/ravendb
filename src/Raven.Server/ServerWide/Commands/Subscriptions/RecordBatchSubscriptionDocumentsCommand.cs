                ...
                if (subscriptionState.ShardingState != null)
                {
                    foreach (var documentRecord in Documents)
                    {
                        ...
                        if (IsAlreadyProcessedReshardedDocument(context, record.Sharding, subscriptionState.ShardingState, documentRecord.DocumentId, documentRecord.ChangeVector))
                        {
                            ...
                            {
                                var exists = TryGetExisting(context, keySlice, out var currentBatch, out var changeVector);
                                if (exists)
                                {
                                    // some newer item from same bucket was already processed but we still have older item in resend list:
                                    // $0 send user/0 & user/1 got failover
                                    // user/0 got reshraded
                        ...
                        // the doc from resharded bucket is mine and it is not already processed and not under active migration, might be from storage or resend list
